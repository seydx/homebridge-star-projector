...
exports.convertColorFromHomeKitToTuya = (obj) => {
  return (
    ...
    (10 * obj.saturation).toString(16).padStart(4, '0') +
    (10 * obj.brightness).toString(16).padStart(4, '0')
  );
};
...
exports.convertColorFromTuyaToHomeKit = (value) => {
  ...
  return {
    hue: parseInt(hue, 16),
    saturation: Math.round(parseInt(saturation, 16) / 10),
    brightness: Math.round(parseInt(brightness, 16) / 10),
  };
};

exports.convertFromScene = (value) => {
  const scene = value.substring(0, 2);
  const seperator = '00000000';
  const mode = value.substring(6, 8);
  const staticMode = mode === '00';
  const flashMode = mode === '01';
  const breathMode = mode === '02';

  const rotationHex = value.substring(2, 6);
  const rotationSpeed = !staticMode ? parseInt(value.substring(2, 4), 16) : 0;

  const colors = value
    .replace(scene, '')
    .split(seperator)
    .filter((val) => val)
    .map((color) => color.replace(`${rotationHex}${mode}`, ''))
    .map((color) => this.convertColorFromTuyaToHomeKit(color));
...
exports.convertToScene = (obj) => {
  let value = '01';
  const rotationHex = `${obj.rotation.toString(16)}${obj.rotation.toString(16)}`;
  const seperator = '00000000';
  const mode = obj.breath ? '02' : obj.flash ? '01' : '00';
  const colors = obj.colors;

  value += rotationHex + mode;

  colors.forEach((color, i) => {
    value += color + seperator;
...
exports.generateTuyaColorFromString = (colors) => {
  ...
  return colors.map((color) => {
    ...
    switch (color) {
      case 'RED':
        tuyaColor = '000003e803e8';
        break;
      case 'BLUE':
        tuyaColor = '00f003e803e8';
        break;
      case 'GREEN':
        tuyaColor = '007803e803e8';
        break;
      case 'ORANGE':
        tuyaColor = '001e03e803e8';
        break;
      case 'YELLOW':
        tuyaColor = '003c03e803e8';
        break;
      case 'PURPLE':
        tuyaColor = '012c03e803e8';
        break;
      case 'CYAN':
        tuyaColor = '00b403e803e8';
        break;
      default:
        tuyaColor = '000003e803e8'; //RED
        break;
    }
